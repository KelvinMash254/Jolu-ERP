...
export default function CustomersPage() {
  ...
  const [form, setForm] = useState({ name: '', phone: '', email: '', county: '', idNumber: '', kraPin: '', physicalAddress: '' });
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setShowForm(false);
      setForm({ name: '', phone: '', email: '', county: '', idNumber: '', kraPin: '', physicalAddress: '' });
        ...
        {showForm && (
          <div className="card mb-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {(['name', 'phone', 'email', 'county', 'idNumber', 'kraPin', 'physicalAddress'] as const).map((field) => (
                <div key={field}>
                  <label className="label capitalize">{field.replace(/([A-Z])/g, ' $1')}</label>
                  <input className="input" value={form[field]} onChange={(e) => setForm({ ...form, [field]: e.target.value })} required={field === 'name' || field === 'phone'} />
                </div>
              ))}
            </div>
            <div className="flex gap-3 mt-4">
              <button onClick={() => createMutation.mutate(form)} className="btn-primary">Save Customer</button>
              ...
              <tbody>
                {customers.map((c) => (
                  <tr key={c.id} className="border-t border-gray-50 hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4">{c.county || '-'}</td>
                    <td className="px-6 py-4">{c.kraPin || '-'}</td>
                    <td className="px-6 py-4">{c.financingBank ? `${c.financingBank} - ${formatCurrency(Number(c.loanAmount || 0))}` : '-'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
